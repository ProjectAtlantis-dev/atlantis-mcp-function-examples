...
async def _generate_image(prompt: str, output_dir: str = "/tmp/marketing_images") -> str:
    """Internal helper to generate image using Pollinations.ai (FREE)"""
...
"""
    ...
    try:
        ...
        image_path = await _generate_image(final_image_prompt)
...
@visible
async def post_to_linkedin(brand_id: str, text: str, image_path: str = ""):
    """
    ...
    Image path: (Optional) Local path to image file to attach.
...
async def generate_and_post_to_linkedin_chrome(brand_id: str, topic: str, include_image: bool = True, image_prompt: str = ""):
    ...
    image_path = result.get("image_path", "") if include_image else ""
...
@visible
async def approve_linkedin_post_chrome(brand_id: str, text: str, image_path: str = ""):
    """
    ...
    post_result = await post_to_linkedin_chrome(brand_id=brand_id, text=text, image_path=image_path)
    logger.info(f"approve_linkedin_post_chrome completed: {len(text)} chars, image={bool(image_path)}")


@visible
async def post_to_linkedin_chrome(brand_id: str, text: str, image_path: str = "", page_url: str = ""):
    """
...
async def generate_and_post_to_linkedin(brand_id: str, topic: str, include_image: bool = True, image_prompt: str = ""):
    ...
    # Read image and encode to base64 if present
    image_base64 = ""
    if image_path and os.path.exists(image_path):
        with open(image_path, 'rb') as f:
            image_data = f.read()
    ...
    let foo = function() {{
        ...
        postBtn.addEventListener('click', async function() {{
            ...
            const data = {{
                ...
                text: text,
                image_path: window.linkedinPostData_{FORM_ID}.image_path
            }};
...
@visible
async def approve_linkedin_post(brand_id: str, text: str, image_path: str = "", replacement_image_base64: str = "", replacement_image_filename: str = ""):
    """
    ...
    If replacement_image is provided, processes it first.
    """
    logger.info(f"approve_linkedin_post called for brand_id={brand_id}, has_replacement={bool(replacement_image_base64)}")

    # If user uploaded a replacement image, process it first
    if replacement_image_base64:
        await atlantis.client_log("🖼️ Processing replacement image...")
        processed_path = await replace_linkedin_image(brand_id, replacement_image_base64, replacement_image_filename)
        if processed_path:
            image_path = processed_path
            logger.info(f"Using replacement image: {image_path}")

    await atlantis.client_log("📤 Posting approved content to LinkedIn...")
    post_result = await post_to_linkedin(brand_id=brand_id, text=text, image_path=image_path)

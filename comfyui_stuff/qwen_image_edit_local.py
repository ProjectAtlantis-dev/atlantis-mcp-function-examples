...
@visible
async def qwen_image_edit(edit_prompt: str = "change hair color to purple"):
    """
    🎨 QWEN IMAGE EDIT
    ...
    async def process_uploaded_image(filename, filetype, base64Content):
        try:
            ...
            # =================================================================
            # STEP 6: DOWNLOAD AND SAVE EDITED IMAGE
            # =================================================================
            ...
            python_server_dir = get_python_server_dir()
            db_path = os.path.join(python_server_dir, "comfyui.db")
            ...
            for node_id, node_output in outputs.items():
                if "images" in node_output:
                    for i, image_info in enumerate(node_output["images"]):
                        ...
                        # Save locally in python-server directory
                        edited_filename = f"{timestamp}_edited_{i}_{filename_from_server}"
                        images_dir = os.path.join(python_server_dir, "comfyui_gallery", "images")
                        local_path = os.path.join(images_dir, edited_filename)
                        ...
                        edit_description = f"Edited: {edit_prompt}"

                        cursor.execute("""
                            INSERT INTO generated_files
                            (id, filename, local_path, workflow_name, prompt_text,
                             server_used, file_size, file_type)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """, (
                            new_file_id,
                            edited_filename,
                        ...
                        await atlantis.client_log(f"💾 Saved: {edited_filename}")
                        await atlantis.client_log(f"🆔 New file ID: {new_file_id}")
                        await asyncio.sleep(0)
            ...
            if saved_files:
                ...
                for image_info in saved_files:
                    ...
                    file_ext = os.path.splitext(image_info["filename"])[1].lower().lstrip('.')
                    ...
                    ">
                        ...
                        <div style="text-align: center;">
                            <img src="data:image/{file_ext};base64,{result_base64}"
                                 style="
                                    max-width: 100%;
                                    height: auto;
                                    border-radius: 10px;
                        ...
                        <div style="
                            ...
                            font-size: 14px;
                        ">
                            🆔 <strong>New File ID:</strong> {image_info["id"]}<br>
                            📏 <strong>File Size:</strong> {image_info["size"]:,} bytes<br>
                            💾 <strong>Saved to:</strong> {image_info["path"]}
                        </div>
                    </div>
                    """

                    await atlantis.client_html(comparison_html)
                    await asyncio.sleep(0)

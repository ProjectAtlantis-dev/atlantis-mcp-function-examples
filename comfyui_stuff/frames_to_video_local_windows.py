...
@visible
async def frames_to_video(prompt: str = "smooth transition between frames", **kwargs):
    """
    ...
    async def process_uploaded_frames():
        try:
            ...
            # Upload both images to ComfyUI server using in-memory buffers
            file_buffer1 = io.BytesIO(file_bytes1)
            file_buffer1.seek(0)
            files = {'image': (temp_filename1, file_buffer1, 'image/png')}
            ...
            file_buffer2 = io.BytesIO(file_bytes2)
            file_buffer2.seek(0)
            files = {'image': (temp_filename2, file_buffer2, 'image/png')}
